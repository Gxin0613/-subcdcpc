...
import { routesFilterPrefix } from '/@/settings/routeFilterSetting';

export interface MultipleTabState {
  cacheTabList: Set<string>;
  tabList: RouteLocationNormalized[];
  lastDragEndIndex: number;
  closedTabList: Set<string>;
}
...
export const useMultipleTabStore = defineStore({
  ...
  getters: {
    ...
    getClosedTabList(): string[] {
      return Array.from(this.closedTabList);
    },
  ...
  actions: {
    ...
    async updateCacheTab() {
      ...
      for (const tab of this.tabList) {
        ...
        // Ignore the cache
        const needCache = item.meta?.enableCache;
        if (!needCache) {
          continue;
        }
    ...
    async removeClosedTab(routePath: string) {
      if (this.closedTabList.size === 0) {
        return;
      }
      this.closedTabList.delete(routePath);
    },
    ...
    async closeTab(tab: RouteLocationNormalized, router: Router) {
      const close = (route: RouteLocationNormalized) => {
        ...
        // 记录被关闭的tab，让页面刷新
        this.closedTabList.add(fullPath);
