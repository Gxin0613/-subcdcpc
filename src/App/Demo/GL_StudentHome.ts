...
export class GL_StudentHome extends PageBaseGenerList {
  override OnTextBlur(_object: Record<string, any>) {
    throw new Error('Method not implemented.');
  }
  constructor() {
    super('GL_StudentHome');
    this.PageTitle = 'GL学生Demo';
  }
  //重写的构造方法.
  async Init() {
    ...
    this.Columns = [
      ...
      { Key: 'XB', Name: '性别', IsShow: true, IsShowMobile: false, DataType: 2 },
    ...
    //处理数据,增加标签.
    ens.forEach((en) => {
      if (en.XB == 0) en.XB = '@女=red';
      else en.XB = '@男=blue';
      en.JinDu = 0.2; //进度设置为0-1小数.
