...
export class GPN_TuiYan extends PageBaseGroupNew {
  constructor() {
    super('GPN_TuiYan');
    this.PageTitle = '推演';
    this.ForEntityClassID = 'TS.Quest.TuiYan';
  }

  public async Init() {
    this.AddGroup('A', '推演', 'icon-drop'); //增加分组.
    const dt = DataType.CurrentDate;
    this.TextBox1_Name('DT', '输入推演的日期', '格式:yyyy-MM-dd', '格式:yyyy-MM-dd', dt, '请入日期:yyyy-MM-dd');
    this.SelectItemsByList('DT.Selected', '选择股票', this.HelpTodo, true, this.MyStocks, true, false, '');
  }
  ...
  public override async Save_TextBox_X(pageNo: string, _sortNo: string, _tb1: string, _tb2: string, _tb3: string) {
    ...
    if (pageNo === 'DT.Selected') {
      const en = new TuiYan();
      en.Name = '推演:' + DataType.CurrentDateTime + ' For:' + this.RequestVal('tb1', 'DT');
