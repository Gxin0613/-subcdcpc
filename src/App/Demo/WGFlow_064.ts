...
export class WGFlow_064 extends WaiGuaBaseFlow {
  ...
  override Init() {
    //根据不同的节点显示不同的按钮.
    if (this.NodeID == 6401) {
      this.ToolbarMyFlow = 'MyFlowBtn'; //工作处理器上的按钮.
      this.ToolbarMyView = 'MyViewBtn'; //工作查看器按钮.
      this.ToolbarMyCC = 'MyCCBtn'; //工作抄送处理器.
    }
    this.ToolbarSearchFlow = 'SearchFlowBtn'; // 流程查询的按钮.
  }
  override BtnClick(btnName: string, _ids?: string) {
    alert(`Button Name:${btnName}, WorkID:${this.WorkID} NodeID:${this.NodeID} SelectIDs:${_ids}`);
    const url = GloComm.UrlGenerList('GL_Todolist');
    return new GPNReturnObj(GPNReturnType.OpenIframeByDrawer40, url);
  }
