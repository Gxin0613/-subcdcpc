 ...
 * 线索
 */
export class XSXiangZhen extends EntityNoName {
  constructor(pkval?: string) {
    super('TS.AD.XSXiangZhen');
  ...
  public override get EnMap() {
    ...
    map.AddTBString('XiangZhen', null, '乡镇', true, false, 0, 100, 200);
    ...
    map.AddHidden('XiangZhen', '=', '@WebUser.DeptNo'); //隐藏的查询条件.
    ...
    rm22.HisMap.AddDDLSysEnum('CLFA', 0, '处理方案', true, false, 'CLFA', '@0=停播@1=立案核查@2=处罚@3=移送司法@4=其他');
    rm22.IsCanBatch = true; //是否可以批处理.
  ...
  public async XiaFa(note: string): Promise<string> {
    ...
    this.XSSta = 3; //下发到区县.
    await this.Update();
    // await Dev2InterfaceCCBill.AddFrmTrackInfo('TS.AD.XS', '广告', this.No, 'Info', '乡镇下发成功', note);
    return '下发成功';
  }
  ...
  public async StartFlow() {
    ...
    //写入日志.
    await Dev2InterfaceCCBill.AddFrmTrackInfo('AD_XS', '线索', this.No, 'Info', '启动流程', '启动流程001');
 ...
 * 线索s
 */
export class XSXiangZhens extends EntitiesNoName {
  get GetNewEntity(): EntityNoName {
    return new XSXiangZhen();
  }
  constructor() {
    super();
  }
