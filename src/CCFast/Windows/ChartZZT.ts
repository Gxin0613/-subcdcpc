...
/// 柱状图
export class ChartZZT extends EntityNoName {
  constructor(pkVal?: string) {
    super('TS.CCFast.Windows.ChartZZT');
    if (!!pkVal) {
      this.setPKVal(pkVal);
    }
  }
  ...
  public override get EnMap() {
    const map = new Map('GPM_WindowTemplate', '柱状图');
    ...
    map.AddTBString(WindowTemplateAttr.PageID, null, 'PageID', false, false, 0, 200, 20, true);
    const help = `
    #### 帮助
    ...
    map.AddBoolean('IsZZT', true, '柱状图', true, false);
    ...
    // const url = GloComm.UrlGPEExt('GPE_GenerDBSrcSearch', '@No', '&MarkID=Group');
    // map.AddLink('C2Ens', '设置-分组数据源', url, false, GPNReturnType.OpenUrlByDrawer50, this.help, 'icon-settings');
    const url1 = GloComm.UrlGPEExt('GPE_GenerDBSrcSearch', '@No', '&MarkID=Main');
    map.AddLink('C3Ens', '设置-明细数据源', url1, false, GPNReturnType.OpenUrlByDrawer50, this.help, 'icon-settings');
    ...
    // // map.AddRM_GPE(new GPE_GenerDBSrcSearch(), 'icon-settings', '', '数据源', '&MarkID=Main');
    // map.AddRM_GPE(new GPE_GenerDBSrcSearch(), 'icon-settings', '', '分组数据源', '&MarkID=Group');
    // map.AddRM_GPE(new GPE_GenerDBSrcSearch(), 'icon-settings', '', '明细数据源', '&MarkID=Dtl');

    // const data = GloGenerDBSrc.GenerData_ByMyPK_WithJson('Rpt_xxxxx_ccc_Main', 'xxxx');
