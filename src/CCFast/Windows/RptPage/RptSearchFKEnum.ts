...
export class RptSearchFKEnumAttr {
  // 字段.
...
/// 外键枚举查询条件
/// </summary>
export class RptSearchFKEnum extends EntityMyPK {
  constructor(pkval?: string) {
    super('TS.RptPage.RptSearchFKEnum');
    if (!!pkval) {
      this.MyPK = pkval;
    }
  }
  ...
  public override get EnMap() {
    ...
    map.AddMyPK();
    map.AddTBString(RptSearchFKEnumAttr.FrmID, null, '表单ID', false, false, 0, 200, 70);
    map.AddTBString(RptSearchFKEnumAttr.KeyOfEn, null, '字段', true, true, 0, 200, 150);
    map.AddTBString(RptSearchFKEnumAttr.Name, null, '字段名', true, true, 0, 200, 150);
    map.AddTBString('UIBindKey', null, '外键或者枚举值', true, true, 0, 200, 150);
    map.AddBoolean('IsEnum', false, '是否枚举?', true, false, false, ''); //是否枚举?
    map.AddDDLStringEnum(RptSearchFKEnumAttr.ShowWay, 'DDL', '显示方式', '@DDL=下拉框@Panel=平铺', true);
    map.AddBoolean(RptSearchFKEnumAttr.IsMultiSelect, false, '多选?', true, true, true);
 ...
 * 外键枚举查询条件s
 */
export class RptSearchFKEnums extends EntitiesMyPK {
  get GetNewEntity(): EntityMyPK {
    return new RptSearchFKEnum();
  }
  constructor() {
    super();
  }
