...
/// 环形图
export class ChartRing extends EntityNoName {
  constructor(pkVal?: string) {
    super('TS.CCFast.Windows.ChartRing');
    if (!!pkVal) {
      this.setPKVal(pkVal);
    }
  }
  ...
  public override get EnMap() {
    const map = new Map('GPM_WindowTemplate', '环形图');
    ...
    map.AddBoolean('IsRing', true, '环形图', true, false);
    ...
    // map.AddTBStringDoc(WindowTemplateAttr.C0Ens, null, '列1外键数据(可选)', true, false, true);
