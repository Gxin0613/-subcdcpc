  ...
  <BaseComponent ref="baseComp" :close-drawer-func="InitPage" :close-modal-func="InitPage">
    <Spin :spinning="loading">
      <div class="list-group">
        <Row type="flex" style="width: 100%; height: 100%" ref="groupRef" v-if="dataScreen.length > 0">
          <Col :span="6 * element.ColSpan" v-for="element in dataScreen" :key="element.No" :data-win-id="element.No">
            <div class="list-group-item">
              <div class="prop_top">
                ...
                <Dropdown v-if="edit" :trigger="['click']">
                  <template #overlay>
                    <Menu @click="handleMenuClick">
                      <MenuItem key="Edit"> <EditOutlined />{{ '编辑' }}</MenuItem>
                      <MenuItem key="Power"> <ThunderboltOutlined />{{ '权限' }}</MenuItem>
        ...
        <Popover :title="'选择向导'">
          <template #content>
            <div class="select-guide">
              <AntButton type="primary" size="small" rounded @click="openAddDrawer('GPN_AIRpt')">{{ '表单AI向导' }}</AntButton>
              <AntButton type="primary" size="small" rounded @click="openAddDrawer('GPN_Windows')">{{ '常规向导' }}</AntButton>
              <!-- <AntButton type="primary" size="small" rounded @click="openAddDrawer('GPN_WindowDBSrc')">{{'数据源向导'}}</AntButton> -->
              <AntButton type="primary" size="small" rounded :disabled="disabledFrmGuide" @click="openAddDrawer('GPN_WindowFrm')">{{ '表单向导' }}</AntButton>
            </div>
          </template>
          <div v-if="edit || params.edit" class="add-btn"> + </div>
        </Popover>
      </div>
    </Spin>
  </BaseComponent>
...
<script lang="ts" setup>
  import { message, Empty, MenuProps, Spin, Dropdown, Menu, MenuItem, Button, Row, Popover, Table, Button as AntButton, Col } from 'ant-design-vue';
  import { FireOutlined, DownOutlined, EditOutlined, ThunderboltOutlined, CloseCircleOutlined, AppstoreOutlined } from '@ant-design/icons-vue';
      ...
      for (const win of dataScreen.value) {
        try {
          ...
          if (win.WinDocModel === WinDocModel.Table) {
            ...
            // const dataC0Ens = await DBAccess.RunSQLReturnTable(win.C0Ens);
            // const keys = Object.keys(data[0]);
            // //获取下拉数据数组dataC0Ens，遍历dataC0Ens并替换数组data中的获取到的枚举值
            // let nonNullData = data.filter((item) => item[keys[0]] !== -1 && item[keys[0]] !== '');
            // dataSouse.value = nonNullData.map((item) => {
            //   const tableItem = dataC0Ens.find((itemC0En) => itemC0En.No === item[keys[0]]);
            //   if (tableItem) {
            //     return {
            //       ...item,
            //       [keys[0]]: tableItem.Name,
            //     };
            //   }
            //   return item;
            // });
            // if (data.length != 0) {
            //   getKey.value = Object.keys(data[0]);
            // }
            // win.tableColums = win.Tag1;
            // getdata.value = win.tableColums.split(',');
            // Columns.value = getKey.value.map((key, index) => {
            //   return {
            //     title: getdata.value[index],
            //     dataIndex: key,
            //     key: key,
            //   };
            // });
  ...
  const InitSortablejs = () => {
    ...
    sortabeljsInst = new Sortable(el, {
      ...
      onEnd: () => {
        const ids = sortabeljsInst?.toArray();
        console.log({ ids });
        // todo 排序后id，需要保存到数据库
      },
    });
