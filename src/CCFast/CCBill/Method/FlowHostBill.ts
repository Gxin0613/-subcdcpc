...
import { FrmBillFlowSingleRoles } from '/@/WF/Admin/FrmLogic/MapExt/FrmBillFlowSingleRole/FrmBillFlowSingleRole';

// 关联表单数据流程
export class FlowHostBill extends EntityNoName {
  constructor(no?: string) {
    super('TS.CCBill.FlowHostBill');
    this.setPKVal(no);
  }
  ...
  public override get EnMap() {
    const map = new Map('Frm_Method', '关联表单数据流程');
    ...
    map.AddRM_DtlSearch('查看权限', new PCenters(), PowerCenterAttr.CtrlPKVal, '', '', '', 'icon-drop', true);
    map.AddRM_DtlSearch('流程规则', new FrmBillFlowSingleRoles(), 'RefPKVal', '', '', 'DBRole,MarkID,MarkName,Docs,IsEnable', 'icon-puzzle', false, '');

    //map.AddRM_DtlSearch('流程规则', new DBRoles(), 'FrmID', '', '', '', 'icon-settings', false, '&DBRole=RecDelete');
    // let rm = new RefMethod();
    // rm.Title = '设计流程';
...
//关联表单数据流程 s
export class MethodFlowHostBills extends EntitiesNoName {
  get GetNewEntity(): EntityNoName {
    return new MethodFlowHostBill();
  }
  constructor() {
    super();
  }
