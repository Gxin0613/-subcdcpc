import { AskBillInside } from './AskBillInside';
...
export class GL_AskBillDtl extends PageBaseGenerList {
  ...
  async BtnClick(btnName: string, _object: Record<string, any>, rowKeys: string) {
    ...
    if (btnName === '分享连接' || btnName === '连接') {
      const mypk = this.RequestVal('MyPK');
      const msg = `连接如下： http://127.0.0.11/#/MyAskFrm?DoType=AskBill&AskMyPK=${mypk}`;
    ...
    if (btnName === '生成连接' || btnName === '连接') {
      ...
      const url = `分享连接:${location.origin}/#/ShareUrl?MyPK=${refMyPK}&FrmID=${frmID}&AskFrmApp=AskBill`;
