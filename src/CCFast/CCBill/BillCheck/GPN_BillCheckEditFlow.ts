...
export class GPN_BillCheckEditFlow extends PageBaseGroupNew {
  constructor() {
    super('GPN_BillCheckEditFlow');
    this.PageTitle = '编辑流程';
  }
  public async Init() {
    this.AddGroup('A', '编辑流程');
    ...
    this.SelectItemsByList('Imp', '请选择要编辑的流程', this.Imp, false, JSON.stringify(list));
  ...
  public override async Save_TextBox_X(pageNo: string, sortNo: string, tb1: string, _tb2: string, _tb3: string) {
    ...
    const url = GloComm.UrlFlowD(tb1);
