        ...
        <div class="search-dict">
          ...
          <div class="search-dict-main" :style="contentStyle">
            <search-toolbar
              ...
              @open-entity-design="openEntityDesign"
            />
...
<script lang="ts" setup>
  ...
  import { useSearchNoName } from './hooks/useSearchNoName';
  ...
  const { getParams, isCreateAction, mapData, initDynamicEntity, query, InitUserRegedit, InitMapAttrs, InitToolbar, updateUserRegedit, toolbarProps, tableConfigs, updateTreeKey } =
    useSearchNoName('BP.CCBill.WF_CCBill', props, 'Search_Init', wrapperRef as any, drawer, displayMode, modalInfo, 'Dict', loading, entityWG as Ref<WaiGuaBaseEntity>);
  const timer = ref(0);
  ...
  const openEntityDesign = () => {
    const url = GloComm.UrlEn('TS.CCBill.FrmEntityNoName', props.params.FrmID);
    wrapperRef.value?.handleGPNCallback(new GPNReturnObj(GPNReturnType.OpenUrlByDrawer75, url + '&FrmID=' + props.params.FrmID, '实体设计'));
  };
  ...
  const InitSearch = async () => {
    try {
      ...
      if (_loaded && !refreshTable.value && !isCreateAction.value) return;
      await initDynamicEntity();
      uac.value = mapData.value.HisUAC;
      refreshTable.value = mapData.value.RowOpenModel == 3;
  ...
  const InitSearchExt = async () => {
    const frmID = getParams('FrmID') || props.params.FrmID;
    const frmNoName = new FrmEntityNoName(frmID);
    await frmNoName.Retrieve();
    console.log('frmNoName.RowOpenModel', frmNoName.RowOpenModel);
    if (frmNoName.RowOpenModel == 2 && !isCreateAction.value) return;
    InitSearch();
