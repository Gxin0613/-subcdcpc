...
const { t } = useI18n();
export class GL_AskFrmRuning extends PageBaseGenerList {
  override OnTextBlur(_object: Record<string, any>) {
    throw new Error('Method not implemented.');
  }
  constructor() {
    super('GL_AskFrmRuning');
    this.PageTitle = '派发工作';
  }
  //重写的构造方法.
  async Init() {
    this.DTFieldOfSearch = 'RDT'; // 按照日期范围查询的字段，为空就不需要日期段查询.
    this.DTFieldOfLabel = '下发日期'; //日期字段名.
    ...
    const data: any = await handler.DoMethodReturnJson('GL_AskFrmRuning');
    //设置数据源.
    this.Data = data;
  }
  ...
  LinkFieldClick(object: Record<string, any>) {
    ...
    if (object.AskFrmApp === 'AskFlow') {
      const paras = `&RefMyPK=${object.RefMyPK}&WorkID=${object.WorkID}&RoutFrom=MyAskFlow&Frm=Todolist&U=${WebUser.No} `;
