...
export class GL_AskFrmPublic extends PageBaseGenerList {
  override OnTextBlur(_object: Record<string, any>) {
    throw new Error('Method not implemented.');
  }
  constructor() {
    super('GL_AskFrmPublic');
    this.PageTitle = '公共活动';
  }
  //重写的构造方法.
  async Init() {
    this.DTFieldOfSearch = 'RDT'; // 按照日期范围查询的字段，为空就不需要日期段查询.
    this.DTFieldOfLabel = '开始日期'; //日期字段名.
    ...
    this.Columns = [
      ...
      { Key: 'DTFrom', Name: '开始', IsShow: true, IsShowMobile: true, DataType: DataType.AppDateTime, width: 150 },
      { Key: 'DTTo', Name: '结束', IsShow: true, IsShowMobile: true, DataType: DataType.AppDateTime, width: 150 },
      { Key: 'RecName', Name: '发起人', IsShow: true, IsShowMobile: true, DataType: DataType.AppString, width: 150 },
    ];

    const handler = new HttpHandler('BP.CCBill.AskFrm.HttpHandlerFrmAsk');
    //handler.AddUrlData();
    handler.AddPara('FrmID', this.RequestVal('FrmID'));
    const data: any = await handler.DoMethodReturnJson('GL_AskFrmPublic');
    //设置数据源.
    this.Data = data;
  }
  ...
  async LinkFieldClick(object: Record<string, any>) {
    ...
    //handler.AddUrlData();
    handler.AddPara('FrmID', this.RequestVal('FrmID'));
    handler.AddPara('MyPk', object.MyPK);
    const no = await handler.DoMethodReturnString('Single_Send');
    let url = '';
    if (object.AskFrmApp === 'AskBill') {
      const paras = `&RefMyPK=${object.MyPK}&FrmID=${object.FrmID}&RoutFrom=MyAskFrm&Frm=Todolist&U=${WebUser.No} `;
      url = GloComm.UrlAskFrm(no, paras);
    }
    if (object.AskFrmApp === 'AskFlow') {
      const paras = `&RefMyPK=${object.MyPK}&WorkID=${no}&RoutFrom=MyAskFlow&Frm=Todolist&U=${WebUser.No} `;
      url = GloComm.UrlMyFlow(object.FrmID, paras);
    }
