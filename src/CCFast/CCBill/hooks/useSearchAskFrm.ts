...
export function useSearchAskFrm(
  className: string,
  props: Recordable,
  queryMethod: string,
  ...
  const tableConfigs = reactive<TableConfig>({
    ...
    onRowClick: (row: Recordable) => {
      ...
      let isReadonly = row.AskFrmState === 3 ? 1 : 0;
      isReadonly = row.RecNo != WebUser.No ? 1 : isReadonly;
  ...
  const InitToolbar = async () => {
    ...
    if (isSearchKey == '') {
      mapData.value.SetPara(FrmAttr.IsSearchKey, '1');
    ...
    if (isSearchKey == '2') {
      const stringSearchKeys = mapData.value.GetValStringByKey('StringSearchKeys') || '';
      if (!!stringSearchKeys) {
        const stringSearchKeysT = mapData.value.GetValStringByKey('StringSearchKeysT') || '';
  ...
  const InitMapAttrs = async () => {
    ...
    let actualAttrs = attrsRef.value.filter((attr) => attr.KeyOfEn != 'No');
    if (showModel == 1) {
      const visibleColSet = new Set<string>(['Name', ...showCols.map((sc) => sc.AttrOfOper)]);
    ...
    if (mapData.value.ListDtlShowWay >= 1) {
      tableConfigs.columns.push({
        ...
        renderExpand: (rowData: RowData) => {
          ...
          return h(SubTableContainer, { style: { width: '600px' }, frmId: FrmID, workId: rowData.No, displayMode: mapData.value.ListDtlShowWay });
        },
      });
    }
    //判断表格内容:换行<->不换行
        ...
        if (attr.KeyOfEn.toLowerCase() === 'name') {
          ...
          cellData.render = (row: Recordable) => {
            const rowMap = new Map([
              [-1, { text: '下发', color: 'orange' }],
              [0, { text: '已访问', color: 'blue' }],
              [1, { text: '草稿', color: 'yellow' }],
              [2, { text: '提交', color: 'green' }],
              [9, { text: '强制结束', color: 'red' }],
            ]);
            const stateInfo = rowMap.get(row['AskFrmState']);
            return h(
              NEllipsis,
              {
                style: {
                  ...
                  return h(
                    ...
                    [
                      h(Tag, { color: stateInfo?.color || '#333' }, () => {
                        if (row['AskFrmState'] === 3) return stateInfo?.text[1] || '未知状态';
