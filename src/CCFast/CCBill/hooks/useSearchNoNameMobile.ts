...
import { FrmBillAttr } from '/@/CCFast/CCBill/FrmBill';
...
export function useSearchNoName(
    ...
    onRowClick: (row: Recordable) => {
      ...
      router.push({
        ...
        query: {
          ...
          IsReadonly: row.EntityState === 100 ? 1 : 0,
          ...props.params,
        },
      });
    },
      ...
      try {
        ...
        router.push({
          ...
          query: {
            ...
            FrmID: data.FrmID,
            FlowNo: data.FlowNo,
            NodeID: parseInt(data.FlowNo) + '01',
          },
        });
      } catch (e) {
    ...
    toolbarProps.buttonList = [
      ...
      /*{
        ...
        onClick: () => {
          ...
          handleBtnClick(MethodModel.Analy);
        },
      },
      {
        key: 'rpt-btn',
        ...
        onClick: () => {
          ...
          handleBtnClick(MethodModel.RPT);
        },
      },*/
      ...btnList
  ...
  const InitMapAttrs = async () => {
    ...
    if (showModel == 1) {
      const cols = ['No', 'Name'].concat(showCols.split(','));
    ...
    if (frmDict.ListDtlShowWay == 1) {
      tableConfigs.columns.push({
        ...
        renderExpand: (rowData: RowData) => {
          ...
          return h(SubTableContainer, { frmId: FrmID, workId: rowData.No });
        },
      });
    }
    // add normal columns
