...
export class GPE_SearchBillMobileShowCol extends PageBaseGroupEdit {
  constructor() {
    super('GPE_SearchBillMobileShowCol');
    this.PageTitle = 'Mobile显示列';
  }
  async Init() {
    this.entity = new FrmDict(); //对应的实体类,就是要对这个实体的枚举值进行编辑.
    this.KeyOfEn = FrmDictAttr.ShowColMobileModel; //要编辑的字段.
    //增加子页面.
    this.AddGroup('A', '显示列');
    this.Blank('0', '所有的列', this.Desc0);
    const sqlGroup = GloWF.sqlGroupField(this.PKVal);
    const sqlFields = GloWF.sqlFields(this.PKVal);
    this.SelectItemsByGroupList('1', '指定的选择列', this.Desc1, true, sqlGroup, sqlFields, FrmDictAttr.ShowMobileCols);
