...
// 数据源实体
export class GLDBView extends EntityNoName {
  constructor(pkval?: string) {
    super('TS.CCBill.GLDBView');
  ...
  public override get EnMap() {
    const map = new Map('Sys_MapData', '不分页视图');
    map.CodeStruct = '4';
    ...
    map.AddRM_GPE(new GPE_GenerDBSrcSearch(), 'icon-settings', '', '数据源(2025)', '&MarkID=Main');
    ...
    rm.Icon = 'icon-energy';
    map.AddRefMethod(rm);
    map.AddRM_GL(new GL_VSTOFrm(), '编辑字段', 'icon-eye', '&FrmID=@No&IsHiddenBtn=1');
