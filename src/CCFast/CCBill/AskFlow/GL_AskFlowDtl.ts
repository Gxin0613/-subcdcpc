...
export class GL_AskFlowDtl extends PageBaseGenerList {
  ...
  async Init() {
    ...
    this.Columns = [
      ...
      { Key: 'WorkID', Name: 'WorkID', IsShow: false, IsShowMobile: false, DataType: 2 },
      { Key: 'FlowNo', Name: 'FlowNo', IsShow: false, IsShowMobile: false, DataType: 2 },
      { Key: 'AskFrmStateDtl', Name: '状态', IsShow: true, IsShowMobile: true, DataType: 1, width: 100 },
  ...
  async BtnClick(btnName: string, _object: Record<string, any>, rowKeys: string) {
    ...
    if (btnName === '表单') {
      const url = GloComm.UrlMyView(_object.WorkID, '&FlowNo=' + _object.FlowNo);
    ...
    if (btnName === '设置') {
      const url = GloComm.UrlEn('TS.AskFrm.AskFlowInside', this.RequestVal('RefMyPK'));
    ...
    if (btnName === '生成连接' || btnName === '连接') {
      ...
      const url = `分享连接:${location.origin}/#/ShareUrl?MyPK=${refMyPK}&FlowNo=${flowNo}&AskFrmApp=AskFlow`;
