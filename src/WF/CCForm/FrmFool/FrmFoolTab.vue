<template>
  <div class="p-5" :class="frmConfigs.frmStyleContent.frmContent" :style="getStyle">
    ...
    <div   :class="fwcRight?'split-layout':''">
      <div :class="FWCGroupFiled==null || fwcRight==false?'pane':'left-pane'" style="background-color: white;">
        <Tabs>
          <!--包含分组的情况-->
          <TabPane v-for="gf in curgfs" :key="gf.OID" :tab="gf['Name' + sysLang] || gf.Lab">
            <MapAttrForm
      ...
      <div v-if="!!FWCGroupFiled && fwcRight"  class="right-pane" style="background-color: white;">
        <div :class="frmConfigs.frmStyleContent.GroupTitle" style="height: 46px;padding: 10px;">
...
import HttpHandler from '/@/utils/gener/HttpHandler';
  ...
  const curgfs = FWCGroupFiled==null || fwcRight.value==false ?props.frmConfigs.gfs: props.frmConfigs.gfs.filter(gf=>gf.CtrlType!='FWC');
  ...
  const SaveWorkCheck=async (isOnlySave)=>{
    ...
    if (props.frmConfigs.isHaveEditWorkCheck == true) {
      if (props.frmConfigs.isHaveSignCheck == true) {
        ...
        //如果有审核组件，Tab标签没有打开审核组件的审批
        if(!workCheckRef.value){
          if(!props.frmConfigs.nodeInfo.FWCDefInfo){
            message.error('请打开审核组件的Tab页签填写审核信息');
            return false;
          }
          try{
            const handler = new HttpHandler('BP.WF.HttpHandler.WF_WorkOpt');
            handler.AddJson(props.params);
            handler.AddPara('Doc', props.frmConfigs.nodeInfo.FWCDefInfo);
