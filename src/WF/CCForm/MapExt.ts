...
export function mapExtParse() {
   ...
   * @constructor
   */
  const GetDataTableByDB = async (mapExt, field, keyVal, pkval, rowData = {}, type = '') => {
    if (!mapExt.FK_DBSrc == null) return [];
    if (mapExt.DBType == 0) {
      const en = new BSEntity('BP.Sys.MapExt', mapExt.MyPK);
      await en.Init();
      const rowDataPara = objectToKeyValueString(rowData);
      //增加表单上的
      const paras = '@Key=' + keyVal.toString().replace(/'/g, '') + rowDataPara;
      const data = await en.DoMethodReturnString('GetDataTableByField', field, paras, '', pkval, type);
    ...
    //处理sql，url参数.
    let dbSrc = mapExt.FK_DBSrc.replace(/~/g, "'");
    if (keyVal != null) {
      keyVal = keyVal.toString().replace(/'/g, '');
  ...
  const GetDataTableOfTBChoice = async (mapExt, pkval) => {
    ...
    switch (parseInt(mapExt?.DoWay)) {
      ...
      case 3: //外键表.
        ...
        const en = new BSEntity('BP.Sys.SFTable');
        en.setPK(tag1);
   ...
   * @constructor
   */
  const GetActionDLLData = async (value, mapExt, field, refPKVal, rowData, type) => {
    let data: any[] = [];
    //不选择的时候
    if (value === '' || value === 'all') {
      data.push({
      ...
      try{
        if(!mapExt.Doc){
          ...
          data = await GloGenerDBSrc.GenerData_ByMyPK_WithJson(mypk,{Key:!!value?.target?.value?value?.target?.value:value,...rowData});
        }else{
          if (!!value?.target?.value) {
            data = (await GetDataTableByDB(mapExt, field, value?.target?.value, refPKVal, rowData, type)) || [];
          } else {
            data = (await GetDataTableByDB(mapExt, field, value, refPKVal, rowData, type)) || [];
          }
        }
  ...
  const GetFullData = async (value, mapExt, refPKVal, rowData, mainData: Record<string, any> | any = null, isDtl = false) => {
    //不选择的时候
    ...
    if (mainData != null) {
      for (const item in mainData) {
        paras = paras.replace('@' + item + '=', '');
        paras += '@' + item + '=' + (mainData[item] || '').toString().replace(/@/g, '[`]');
      }
    }
    if (!value) paras += '@Key=';
    else {
      if (value.toString().includes(',')) value = value.replace(/,/g, "','");
