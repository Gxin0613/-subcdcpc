...
export class GPN_DtlImpExcel extends PageBaseGroupNew {
  constructor() {
    super('GPN_DtlImpExcel'); //实体的类名，以GPE_开头.
  ...
  public async Init() {
    ...
    //增加子页面分组.
    this.AddGroup('A', '导入');
    this.FileUpload('0', '清空方式导入', '请上传符合格式的Excel文件.', this.Desc0);
  ...
  public override async Save_TextBox_X(pageID: string, _sortNo: string, _tb1: string, _tb2: string, _tb3: string) {
    ...
    const data = await handler.DoMethodReturnJson<Recordable>('DtlImpByExcel_Imp');
