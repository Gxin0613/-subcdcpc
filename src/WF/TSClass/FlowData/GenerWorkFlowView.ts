...
import { GPN_FlowDataRecJuggle } from '/@/WF/TSClass/FlowData/GPN_FlowDataRecJuggle';
import { message } from 'ant-design-vue';

// 流程实例查询
export class GenerWorkFlowView extends EntityWorkID {
  constructor(pkval?: number) {
    super('TS.FlowData.GenerWorkFlowView');
  ...
  public override get EnMap() {
    ...
    map.AddDDLSysEnum(
      GenerWorkFlowViewAttr.WFState,
      0,
      '大状态',
      true,
      false,
      GenerWorkFlowViewAttr.WFState,
      '@0=空白@1=草稿@2=进行中@3=已完成@4=挂起@5=退回@6=移交@7=作废@8=加签',
    );
    ...
    map.AddTBString('Emps', null, 'Emps', false, false, 0, 30, 70);

    //单组织版
    if (WebUser.CCBPMRunModel == CCBPMRunModel.Single) {
      if (WebUser.IsAdmin == false) map.AddHidden('Emps', ' LIKE ', '%@WebUser.No%');
    } else {
    ...
    if (WebUser.IsAdmin) {
      //管理处理人.
      map.AddRM_GL(new GL_AdminGenerList(), '管理处理人', 'icon-people', '&NodeID=@FK_Node'); //管理处理人.
      ...
      map.AddRM_GPN(new GPN_FlowDataRecJuggle(), 'icon-people'); // 数据篡改
  ...
  public async AddViewer(p1: string, p2: string): Promise<string> {
    const en = new BSEntity('BP.WF.GenerWorkFlow', this.WorkID);
    await en.Retrieve();
    const str = await en.DoMethodReturnString('Shift', p1, p2);
    return str.replace(/%40/g, '@');
  ...
  public async DoFlowOver(note: string): Promise<string> {
    ...
    await en.Init();
    await en.RetrieveFromDBSources();
    const msg = await en.DoMethodReturnString('DoFlowOver', note);
    if (typeof msg === 'string' && msg.includes('err@')) {
      message.error(msg.replace('err@', ''));
...
//流程实例查询s
export class GenerWorkFlowViews extends EntitiesWorkID {
