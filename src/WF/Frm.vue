    ...
    <Spin :spinning="loading">
      ...
      <div v-else style="background-color: rgb(240, 242, 245); height: 100%">
        ...
        <div class="content wrapper" :style="contentStyle">
          <ChartFrm v-if="pagetype == 10 && frmData" :params="query" ref="ChartFrmRef" :field-is-readonly="params.fieldIsReadonly" />
          <FrmFool v-if="pagetype === 0 && frmData" ref="basicData" :frmData="frmData" :fieldIsReadonly="params.fieldIsReadonly" :isReadonly="isReadonly" :params="query" />
          <FrmSelf v-if="pagetype === 3 && frmData" ref="basicData" :frmData="frmData" :fieldIsReadonly="params.fieldIsReadonly" :isReadonly="isReadonly" :params="query" />
        </div>
      </div>
    </Spin>
  </div>
...
<script lang="ts" setup>
  ...
  const InitPage = async () => {
    try {
      loading.value = true;
      isReadonly.value = props.params.isReadonly || props.isReadonly;
      //通过Component使用页面，从props.params取参数
