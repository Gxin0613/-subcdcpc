<template>
  <BaseComponent ref="baseComp" :closeDrawerFunc="editClosed" :close-modal-func="editClosed">
    <div ref="dtlBatchRef" class="dtl-batch-wrapper">
      <!-- 表格头部工具栏 -->
      <div class="table-toolbar">
        ...
        <div class="toolbar-right">
          <Button v-for="btn in actionBtnsRef" :ghost="btn.ghost" :disabled="gloReadonly" :key="btn.label" @click="btn.onClick" :class="['dtl-batch-btn', btn_style(btn.label)]">
            <i :class="getBtnIcon(btn.label)"></i>
            {{ btn.label }}
          </Button>
          <Button v-for="btn in enArrMethods" ghost :key="btn.label" :disabled="gloReadonly" class="dtl-batch-btn ext-btn" @click="execEnArrMethods(btn)">
            <i class="icon-extension"></i>
            {{ btn.label }}
          </Button>
          <Button v-for="btn in extMethods" ghost :key="btn.label" :disabled="isExtMethodReadonly(btn)" class="dtl-batch-btn ext-btn" @click="execExtMethods(btn)">
            <i class="icon-gear"></i>
            {{ btn.label }}
          </Button>
        </div>
      ...
      <div class="table-container">
        <BasicTable v-if="tableConfig" class="dtl-batch-table" @register="tableConfig" :scroll="{ y: props.height }">
            ...
            <div class="cell-wrapper" :class="{ 'cell-error': isValidFailed(attr, row) }">
              ...
              <template v-else-if="isBoolean(attr)">
                <div class="switch-wrapper">
                  <AntSwitch v-model:checked="row[attr.Key]" :disabled="mixedReadonly(attr, row)" :style="mergeAttrStyle(attr)" class="dtl-batch-switch">
              ...
              <template v-else-if="isDate(attr)">
                <DatePicker
                  ...
                  @change="onContentChange(row, attr.Key)"
                  class="dtl-batch-date-picker"
                />
              </template>
              ...
              <template v-else-if="isDateTime(attr)">
                ...
                <DatePicker
                  ...
                  :disabled="mixedReadonly(attr, row)"
                  @change="onContentChange(row, attr.Key)"
                  class="dtl-batch-datetime-picker"
                />
              </template>
              ...
              <template v-else-if="isTextBox(attr)">
                ...
                <div v-else-if="attr.Key.toLowerCase().includes('color')" class="color-picker-wrapper">
                  <NColorPicker v-model:value="row[attr.Key]" show-alpha class="dtl-batch-color-picker" />
                </div>
              ...
              <template v-else-if="isPopTextArea(attr)">
                <div class="pop-selector-wrapper">
                  <InputGroup compact :disabled="true" :style="mergeAttrStyle(attr)">
                    <Input style="display: none" v-model:value="row[attr.Key]" @change="onContentChange(row, attr.Key)" />
                    <div class="pop-display-area" :id="'div_' + attr.Key" :class="{ readonly: mixedReadonly(attr, row) }">
                      <div class="tags-container">
                        <template v-if="!row[attr.Key + 'T']">
                          <Tag v-if="row[attr.Key]" class="dtl-batch-tag">{{ row[attr.Key] }}</Tag>
                        </template>
                        <template v-else>
                          <Tag
              ...
              <template v-else-if="isRadioButton(attr)">
                <RadioGroup
                  ...
                  :style="mergeAttrStyle(attr)"
                  :disabled="mixedReadonly(attr, row)"
                  class="dtl-batch-radio-group"
                >
                  <Radio v-for="option in attr.ddl" :key="option.value" :value="option.value" class="dtl-batch-radio">
                    {{ option.label }}
                  </Radio>
                </RadioGroup>
              </template>
              ...
              <template v-else-if="isSingleAth(attr)">
                <div class="upload-wrapper">
                  <Button @click="openFieldAth(attr, row[entity!.PK], attr.UIIsReadonly || props.readonly)" class="dtl-batch-upload-btn">
                    <UploadOutlined />
                    上传/查看
                  </Button>
                </div>
        ...
        <Alert message="提示" description="此从表需要与主表id关联, 请先保存主表" type="info" class="disabled-alert">
          <template #action>
            <Space direction="vertical">
              <Button size="small" type="primary" @click="$emit('save-main-table')" class="dtl-batch-btn"> 保存并使用 </Button>
            </Space>
          </template>
        </Alert>
      </div>
      ...
      <template #footer>
        <div class="modal-footer">
          ...
          <div class="button-group">
            <Button key="back" @click="popModalCancel" class="dtl-batch-btn secondary">取消</Button>
            <Button key="submit" type="primary" class="dtl-batch-btn primary" @click="PopModalOK">确定</Button>
          </div>
        </div>
      </template>
      <Pop
    ...
    <drawer v-model:open="rightDrawer.visible" width="70%" :title="rightDrawer.title" @close="editClosed(true)" :body-style="bodyStyle" class="dtl-batch-drawer">
      <component
...
<script lang="ts" setup>
  ...
  const { EnName, ShowAttrs = '', PKVal, ButsTableTop, IsSummary /*ButsItem*/ } = toRaw(props.params);
  const Data = ref<any>([]);
  ...
  const InitPage = async () => {
    try {
      ...
      entity.value.SetPageParam(props.mainTableInfo ?? {});
      // 外部参数
      const _enMap = entity.value.EnMap;
      ...
      // 设置基础配置
      const attrs = _enMap.attrs;
      HisUAC.value = entity.value.HisUAC;
      tableColumns.value = [];
      // 构建列配置
    ...
    Modal.confirm({
      ...
      async onOk() {
        try {
          ...
          await reload_func.value?.();
          if (editTableEvents.value?.onDelete) {
            editTableEvents.value.onDelete(get_db_source_func.value?.() || []);
          }
  ...
  const createTable = async () => {
    ...
    const [registerTable, { reload, setLoading, getDataSource, clearSelectedRowKeys }] = useTable(config);
...
<style lang="less" scoped>
  ...
  .dtl-batch-date-picker,
  .dtl-batch-datetime-picker,
  .dtl-batch-time-picker {
    :deep(.ant-input),
  ...
  // 各种输入组件的现代化样式
  .dtl-batch-input,
  .dtl-batch-number-input,
  .dtl-batch-datetime-picker,
  .dtl-batch-select,
  .dtl-batch-autocomplete {
    :deep(.ant-input),
