...
const CLASS_PREFIX = '/DataV_';
const allTsInfo = import.meta.glob('/src/**/*.ts');
const dataVTsKeys = Object.keys(allTsInfo).filter((key) => key.includes(CLASS_PREFIX));
const modules: Recordable = {};
dataVTsKeys.forEach((key) => {
  modules[key] = allTsInfo[key];
});

export class ClassFactoryOfDataV {
  public static async GetEn(classID: string): Promise<DataVBase> {
    // 如果是全路径匹配
    classID = dealClassId(classID, CLASS_PREFIX);
    const fileSrc = dataVTsKeys.find((key) => key.includes(classID + '.ts'));
    if (!fileSrc) {
      throw new Error(`没有找到 DataV class - [${classID}]`);
    }
    const moduleLoader = modules[fileSrc];
    if (!moduleLoader) {
      throw new Error('DataV-GetEn 没有找到文件:' + fileSrc);
    }
    const clsModule = (await moduleLoader()) as object;
    const jsModule = clsModule[classID];
    if (!jsModule) {
      const keys = Object.keys(clsModule);
      const message = `DataV-GetEn 文件 [${fileSrc}] 中 没有正确导出名为 ${classID} 的模块，现有模块名为 [${[...keys]}]  请检查`;
      alert(message);
  ...
  public static async toJSON(filteredPrefix: string[]) {
    const targetClsList = dataVTsKeys
