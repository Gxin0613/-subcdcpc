...
// 表单类工厂
export class ClassFactoryOfWaiGuaFlow {
  public static async GetEn(classID: string): Promise<WaiGuaBaseFlow | null> {
    // 如果是全路径匹配
    ...
    if (!moduleLoader) {
      throw new Error('WaiGuaFlow-GetEn 没有找到文件:' + fileSrc);
    }
    const clsModule = (await moduleLoader()) as object;
    const jsModule = clsModule[classID];
    if (!jsModule) {
      const keys = Object.keys(clsModule);
      const message = `WaiGuaFlow-GetEn 文件 [${fileSrc}] 中 没有正确导出名为 ${classID} 的模块，现有模块名为 [${[...keys]}]  请检查`;
      alert(message);
