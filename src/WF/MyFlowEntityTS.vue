  ...
  <div style="background-color: rgb(240, 242, 245); height: 100%">
    <Spin :spinning="loading">
      ...
      <div v-else style="background-color: rgb(240, 242, 245); height: 100%">
        <!--工具栏-->
        <div class="header toolBar" :style="{ minHeight: '55px' }">
          <ToolBar :pageType="query.PageType" :params="query" @ChangeLoading="ChangeLoading" @Save="Save" />
        </div>
        <!--表单内容-->
...
<script lang="ts" setup>
  ...
  const InitPage = async () => {
    try {
      ...
      if (fn.WhoIsPK === 2) {
        const gwf = new GenerWorkFlowExt(query.WorkID);
        await gwf.RetrieveFromDBSources();
        query.PKVal = gwf.PWorkID;
      }
