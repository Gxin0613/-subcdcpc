...
// 表格弹窗
export class GPEAutoFullDDLSFTable extends EntityMyPK {
  constructor(mypk?: string) {
    super('TS.MapExt.GPEAutoFullDDLSFTable');
  ...
  public override get EnMap() {
    const map = new Map('Sys_MapExt', '填充过滤');
    map.GroupBarShowModel = 1;
    map.AddGroupAttr('基本设置');
    map.AddMyPK();

    map.AddTBString(MapExtAttr.FK_MapData, null, '表单ID', true, true, 0, 50, 200);
    map.AddTBString(MapExtAttr.ExtModel, null, 'AutoFull', false, false, 0, 50, 200);
    map.AddTBString(MapExtAttr.ExtType, null, 'AutoFull', false, false, 0, 50, 200);
    map.AddTBString(MapExtAttr.AttrOfOper, null, '当前字段', true, true, 0, 50, 200);

    //const sql1 = `SELECT No,Name FROM Sys_SFTable WHERE No!='Blank' AND IsPara=0 `; //查询出来有参数的字典. IsPara=0
    map.AddDDLSQL(MapExtAttr.Doc, null, '字典表', GloWF.SQLOfActiveDDLDoc, true);
    ...
    // map.AddRM_DtlSearch('参数设置', new SFParaSettings(), 'RefPK', '检查');
