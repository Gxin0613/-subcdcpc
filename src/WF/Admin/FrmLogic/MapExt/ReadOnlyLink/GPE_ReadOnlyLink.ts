...
export class GPE_ReadOnlyLink extends PageBaseGroupEdit {
  constructor() {
    super('GPE_ReadOnlyLink');
    this.PageTitle = '字段值链接';
  }
  async Init() {
    ...
    //初始化数据.
    await this.entity.InitDataForMapAttr('ReadOnlyLink', this.GetRequestVal('PKVal'));

    //增加子页面.
    this.AddGroup('A', '字段值链接'); //增加分组.
    ...
    this.AddEntity('UrlOpen', '模态弹窗', new LinkAttr(), this.UrlOpen, '');
    this.AddEntity('UrlWinOpen', '新窗口弹出url', new LinkAttr(), this.UrlWinOpen, '');
  }
