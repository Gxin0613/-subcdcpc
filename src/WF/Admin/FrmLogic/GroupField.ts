...
// SQL模板属性
export class GroupField extends EntityOID {
  constructor(pkval?: number) {
    super('TS.FrmUI.GroupField');
  ...
  public override get EnMap() {
    ...
    // map.AddTBInt('TableCol', 0, '显示46列', true, false);
    ...
    rm.ClassMethod = 'DoDelAllField';
    map.AddRefMethod(rm);

    const rm1 = new RefMethod();
    rm1.Title = '章节表单分组扩展';
    rm1.HisMap.AddTBInt('type', 0, '设置类型：0链接到其它表单，1自定义URL', true, false);
    rm1.HisMap.AddTBString('val', null, '输入对应的值', true, false, 0, 1000, 1000);
    rm1.ClassMethod = 'DoSetGFType';
    rm1.RefMethodType = RefMethodType.Func;
    map.AddRefMethod(rm1);

    // const rm2 = new RefMethod();
    // rm2.Title = '调整字段顺序';
    // //  rm2.ClassMethodName = this.ToString() + '.DoGroupFieldIdx';
...
// 表单注册s
export class GroupFields extends EntitiesOID {
