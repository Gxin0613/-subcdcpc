...
export class GPN_DBSrcSQL extends PageBaseGroupNew {
  constructor() {
    super('GPN_DBSrcSQL');
    this.PageTitle = '新建';
  }
  public Init() {
    ...
    this.TextBox3_NameNoNote('Procedure', '创建过程', this.HelpSearch, 'S', '编号', '名称', '备注', '我的过程');
    this.AddIcon('icon-options', 'Procedure');
  }
  ...
  public override async Save_TextBox_X(pageNo: string, sortNo: string, tb1: string, tb2: string, tb3: string) {
    ...
    if (pageNo == 'SQL.Doc.CodeStruct') {
      ...
      sfTable.FK_SFDBSrc = this.RefPKVal; //'local'; // this.RequestVal('tb1', 'SQL.Doc'); //数据源.
      sfTable.FK_Val = sfTable.No;
      await sfTable.Insert();
