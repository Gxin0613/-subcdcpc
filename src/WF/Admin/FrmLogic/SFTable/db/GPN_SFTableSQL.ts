...
export class GPN_SFTableSQL extends PageBaseGroupNew {
  constructor() {
    super('GPN_SFTableSQL');
    this.PageTitle = '新建字典';
  }
  public Init() {
  ...
  public override async Save_TextBox_X(pageNo: string, sortNo: string, tb1: string, tb2: string, tb3: string) {
    ...
    if (pageNo == 'SQL.Doc.CodeStruct') {
      ...
      sfTable.FK_SFDBSrc = this.RefPKVal; // this.RequestVal('tb1', 'SQL.Doc'); //数据源.
      alert(sfTable.FK_SFDBSrc);
