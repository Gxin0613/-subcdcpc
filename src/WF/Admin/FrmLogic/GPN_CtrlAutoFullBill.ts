...
export class GPN_CtrlAutoFullBill extends PageBaseGroupNew {
  constructor() {
    super('GPN_CtrlAutoFullBill');
    //  this.ForEntityClassID = 'TS.FrmUI.SFTable';
    this.PageTitle = '引用单据';
  }
  public async Init() {
    this.AddGroup('A', '内置', 'icon-list'); //增加分组.
    this.SelectItemsByGroupList('FrmBillInput', '单据引用', this.InputEmpDesc, false, GloWF.srcFrmTree, GloWF.srcFrmListOfBill);
    this.SelectItemsByList('FrmBillInput.Fields', '引入字段', this.InputEmpDesc, true, this.GenerTableFields);
  }
  ...
  public override async Save_TextBox_X(pageNo: string, _sortNo: string, tb1: string, tb2: string, _tb3: string) {
    ...
    if (pageNo == 'FrmBillInput.Fields') {
      const frmFrmID = this.RequestVal('tb1', 'FrmBillInput');
      return await this.FrmInput(frmID, frmFrmID, _sortNo, 'BillNo', 'Title', tb1, tb2);
    }
  }
