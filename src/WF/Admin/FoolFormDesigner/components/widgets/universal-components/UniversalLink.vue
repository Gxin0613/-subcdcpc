<template>
  <select-helper :widget="widgetInfo" :setting-url="settingUrl">
    <n-form-item :label="widgetInfo.title" :show-feedback="false">
      <div class="item">
        <a :href="widgetInfo.modelVal" target="_blank" rel="noopenner noreferrer">{{ '链接跳转' }}</a>
      </div>
    </n-form-item>
  </select-helper>
</template>
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      return {
        label,
        InputTheme,
        settingUrl: `../../Comm/EnOnly.htm?EnName=TS.FrmUI.ExtLink&PKVal=${props.widgetInfo?.id}&s=${Math.random()}`,
      };
    },
  });
</script>
