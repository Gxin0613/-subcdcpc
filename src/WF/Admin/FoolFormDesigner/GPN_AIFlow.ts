...
export class GPN_AIFlow extends PageBaseGroupNew {
  constructor() {
    super('GPN_AIFlow');
    this.PageTitle = 'AI流程';
  }
  public async Init() {
    ...
    const words2 = '创建请假表单，包括但不限于如下字段：请假日期从、到、请假天数请假原因。';
    this.TextArea('AIFlow.Frm', '输入表单要求', this.WordAlert, '提示词', words2, '请点击帮助参考如何输入提示词?');

    this.FileUpload('File', '根据文件生成表单', '请上传Excel,word,图片文件.', this.HelpFile);
...
//    debugger;
    ...
    if (_pageNo == 'AIFlow.Frm') {
      ...
      if (this.RequestVal('SystemNo') != '') {
        ...
        en.ModuleNo = this.RequestVal('ModelNo');
        en.SystemNo = this.RequestVal('SystemNo');
