...
export class GPN_NewFlow extends PageBaseGroupNew {
  constructor() {
    super('GPN_NewFlow');
    this.PageTitle = '新建流程';
  }
  public async Init() {
    ...
    this.TextBox1_Name('RefOneFrmTree', '绑定单表单', GPN_NewFlow.RefFrm, '流程名称', '我的单表单流程');
    ...
    this.SelectItemsByGroupList('FrmTree.SelectFrms', '选择表单', this.FrmTree_SelectFrms, true, GloWF.srcFrmTree, GloWF.srcBindFrmList);
    this.AddGroup('D', '使用高代码');
    this.TextBox1_Name('EntityTS', '绑定高代码', this.EntityTS, '流程名称', '我的高代码流程');
    this.AddIcon('icon-star', 'EntityTS');
    this.SelectItemsByList('EntityTS.SelectOneEntityTS', '选择Entity实体', this.EntityTS_SelectOneEntity, false, await this.GenerEnsList());
  ...
  public override async Save_TextBox_X(pageID: string, sortNo: string, tb1: string, tb2: string, tb3: string) {
    ...
    if (pageID === 'Profession' || pageID === 'JiJian') {
      ...
      const flowNo = await GPN_NewFlow.creteFlow(sortNo, model, flowName, FrmUrl, tb3);
    ...
    if (pageID == 'SDKFrmWorkID' || pageID == 'SDKFrmWorkID' || pageID == 'QianRuFrm') {
      const flowName = tb2;
      const FrmUrl = tb1;
      const flowNo = await GPN_NewFlow.creteFlow(sortNo, FlowDevModel.SDKFrm, flowName, FrmUrl, tb3);
      if (flowNo == null) return;
    ...
    if (pageID === 'RefOneFrmTree.SelectOneFrm') {
      ...
      const frmID = tb1;
      const flowNo = await GPN_NewFlow.creteFlow(sortNo, FlowDevModel.RefOneFrmTree, flowName, frmID, tb3);
      if (flowNo == null) return;
      // const url = '/#/WF/Designer/EditFlow?FlowNo=' + flowNo;
      const url = GloComm.UrlFlowD(flowNo);
    ...
    if (pageID === 'FrmTree.SelectFrms') {
      ...
      const frmID = tb1;
      const flowNo = await GPN_NewFlow.creteFlow(sortNo, FlowDevModel.FrmTree, flowName, frmID, tb3);
      if (flowNo == null) return;
    ...
    if (pageID === 'EntityTS.SelectOneEntityTS') {
      const flowName = this.RequestVal('tb1', 'EntityTS');
      const frmID = tb1;
      const flowNo = await GPN_NewFlow.creteFlow(sortNo, FlowDevModel.EntityTS, flowName, frmID, tb3);
      if (flowNo == null) return;
      //  const url = '/#/WF/Designer/EditFlow?FlowNo=' + flowNo;
      const url = GloComm.UrlFlowD(flowNo);
  ...
  public static async creteFlow(sortNo: string, devModel: FlowDevModel, flowName: string, frmID_URL: string, frmPK: string) {
    ...
    const data = await handler.DoMethodReturnString('FlowDevModel_Save');
  ...
  #### 帮助
   ...
   `;
  // 专业模式
  public static readonly Profession = `       
  #### 帮助
...
`;
  ...
  `;
  public static readonly RefFrm_SelectOneFrm = `
  #### 帮助  
   - 表单库的表单称为独立表单，每个表单都可以与任何流程的任何节点绑定。
   - 一个表单类似于一个车厢，停留在仓库里，只有他在被绑定到节点上才可以使用。
   - 每个节点就类似于火车头，一个火车头可以挂多个车厢，也可以挂一个车厢。
  ...
  ![输入图片说明](./resource/WF/Admin/Img/Frms.jpg "屏幕截图.png")    
  `;
  public static readonly FrmTree_SelectFrms = `
  #### 帮助
   - 表单库的表单称为独立表单，每个表单都可以与任何流程的任何节点绑定。
   - 一个表单类似于一个车厢，停留在仓库里，只有他在被绑定到节点上才可以使用。
   - 每个节点就类似于火车头，一个火车头可以挂多个车厢，也可以挂一个车厢。
  ...
  #### 图例
  `;
  public readonly EntityTS_SelectOneEntity = `
  #### 帮助  
