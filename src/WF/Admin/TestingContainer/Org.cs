    ...
    public class Org : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                rm.HisMap.AddTBString("adminer", null, "新主管理员编号", true, false, 0, 100, 100);
                map.AddRefMethod(rm);
                ...
                rm.HisMap.AddTBString("no", null, "组织编号", true, false, 0, 100, 100);
                rm.HisMap.AddTBString("name", null, "组织名称", true, false, 0, 100, 100);

                rm.HisMap.AddTBString("adminer", null, "管理员编号", true, false, 0, 100, 100);
                rm.HisMap.AddTBString("adminName", null, "管理员名称", true, false, 0, 100, 100);
                ...
                rm.HisMap.AddTBString("adminer", null, "部门编号", true, false, 0, 100, 100);
                map.AddRefMethod(rm);
        ...
        public string AddOrg(string orgNo, string orgName, string adminerNo, string adminerName)
        {
            ...
            try
            {
                ...
                en = new BP.Sys.FrmTree();
                en.Copy(ft);
                en.No = DBAccess.GenerOID().ToString();
                en.Name = "办公表单";
                en.ParentNo = ft.No;
                en.SetValByKey("OrgNo", this.No);
                en.Insert();
        ...
        protected override bool beforeInsert()
        {
            ...
            if (org.RetrieveFromDBSources() != 0)
                throw new Exception("err@组织编号:" + this.No + "已存在,请重新输入组织编号");
            if (DataType.IsNullOrEmpty(this.Adminer) == false)
            {
                BP.Port.Emp emp = new BP.Port.Emp();
                emp.No = this.Adminer;
                if(emp.RetrieveFromDBSources()!= 0 && DataType.IsNullOrEmpty(emp.OrgNo)==false
                    && emp.OrgNo.Equals(this.No)==false)
                    throw new Exception("err@该人员:" + this.Adminer + "已经在其他组织中存在");
                //二级管理员不存在。插入一条信息
                if (emp.RetrieveFromDBSources() == 0)
                {
                    //增加人员。
                    emp.Name = this.AdminerName;
                    emp.FK_Dept = this.No;
                    emp.OrgNo = this.No;
                    emp.Pass = "123";
                    emp.Insert();
        ...
        {
            //如果是已经存在的部门设置为组织编号，则修改部门中人员的组织编号
            Dept dept = new Dept();
            dept.No = this.No;
            ...
            else
            {
                ...
                mydept.ParentNo = dept.No;
                mydept.No = this.No;
                mydept.Name = this.Name;
                mydept.OrgNo = this.No;
                mydept.Insert();
            }
            if (DataType.IsNullOrEmpty(this.Adminer) == false)
            {
                BP.Port.Emp emp = new BP.Port.Emp(this.Adminer);
                //增加管理员.
                OrgAdminer oa = new OrgAdminer();
                oa.FK_Emp = emp.No;
                oa.OrgNo = this.No;
        ...
        protected override bool beforeUpdateInsertAction()
        {
            ...
            this.SetValByKey("FlowNums", GloWF.SQLOfFlowNums);//DBAccess.RunSQLReturnValInt("SELECT COUNT(*) AS a FROM WF_Flow WHERE OrgNo='" + WebUser.OrgNo + "'"));
            this.SetValByKey("FrmNums", GloWF.SQLOfFrmNums);//DBAccess.RunSQLReturnValInt("SELECT COUNT(*) AS a FROM Sys_MapData WHERE OrgNo='" + WebUser.OrgNo + "'"));

            this.SetValByKey("Users", GloWF.SQLOfUsers);//DBAccess.RunSQLReturnValInt("SELECT COUNT(*) AS a FROM Port_Emp WHERE OrgNo='" + WebUser.OrgNo + "'"));
            this.SetValByKey("Depts", GloWF.SQLOfDepts);//DBAccess.RunSQLReturnValInt("SELECT COUNT(*) AS a FROM Port_Dept WHERE OrgNo='" + WebUser.OrgNo + "'"));
            this.SetValByKey("GWFS", GloWF.SQLOfGWFS);//DBAccess.RunSQLReturnValInt("SELECT COUNT(*) AS a FROM WF_GenerWorkFlow WHERE OrgNo='" + WebUser.OrgNo + "' AND WFState!=3"));
            this.SetValByKey("GWFSOver", GloWF.SQLOfGWFSOver);//DBAccess.RunSQLReturnValInt("SELECT COUNT(*) AS a FROM WF_GenerWorkFlow WHERE OrgNo='" + WebUser.OrgNo + "' AND WFState=3"));
