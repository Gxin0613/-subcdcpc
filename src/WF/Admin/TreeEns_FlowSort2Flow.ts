 ...
 * 流程类别流程
 */
export class TreeEns_FlowSort2Flow extends PageBaseTreeEns {
  constructor() {
    super('TreeEns_FlowSort2Flow');
    this.PageTitle = '流程模板';
  }
  ...
  override async Init() {
    ...
    // 定义列，这些列用于显示.
    this.Columns = [
      { id: FlowAttr.No, name: '编号', width: 100 },
      { id: FlowAttr.Name, name: '名称', width: 300 },
      { id: FlowAttr.PTable, name: '存储表', width: 150 },
      { id: 'FlowDevModelText', name: '模式', width: 150 },
      { id: FlowAttr.IsCanStart, name: '启动?', dataType: DataType.AppBoolean, width: 100 },
      { id: 'CreateDate', name: '创建日期', width: 150, dataType: DataType.AppDate },
      // { id: 'FrmUrl', name: '表单关联', width: 300 },
    ];
    ...
    this.BtnsOfTableTop = '新建,AI创建,导入,目录属性';
  ...
  public override async GetDtls(nodeID: string) {
    ...
    await flows.Retrieve('FK_FlowSort', nodeID, 'Idx');
    return flows;
  }
  ...
  //   return GloComm.UrlEnOnly('TS.WF.Template.FlowExt', _row[_pk]);
  // }
  ...
  public async BtnClick(btnLab: string, treeNodeID: string, itemIDs: string) {
    ...
    if (btnLab === '新建流程' || btnLab === '新建') {
      const url = GloComm.UrlGPN('GPN_NewFlow', '&SortNo=' + treeNodeID);
      return new GPNReturnObj(GPNReturnType.OpenUrlByDrawer, url, '新建流程');
    }
