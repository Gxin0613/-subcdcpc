<template>
  <div class="content">
    <BasicTable @register="registerTable" :pagination="false" style="height: calc(100vh)">
...
<script lang="ts" setup>
  ...
  const InitPage = async () => {
    try {
      ...
      await ens.Init();
      let data = '';
      if (props.attrKey == 'WorkCheck') {
        data = await ens.DoMethodReturnJSON('InitData_Flow');
      } else if (props.attrKey == 'ReturnFlow') {
        data = await ens.DoMethodReturnJSON('InitData_ReturnFlow');
      }
      console.log(data);
