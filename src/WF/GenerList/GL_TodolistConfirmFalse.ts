...
export class GL_TodolistConfirmFalse extends PageBaseGenerList {
  constructor() {
    super('GL_TodolistConfirmFalse');
    this.PageTitle = '未确认待办';
  }
  //重写的构造方法.
  async Init() {
    ...
    if (!!flowNo) {
      ...
      if (flow.BuessFieldRole == 1) {
        isShowColumn = false;
        this.Columns = [
          { Key: 'WorkID', Name: '工作ID', IsShow: false, IsShowMobile: false, DataType: DataType.AppInt },
          { Key: 'Title', Name: '标题', IsShow: true, IsShowMobile: true, DataType: DataType.AppString, width: 350 },
          { Key: 'FlowStarterName', Name: '发起人', IsShow: true, IsShowMobile: true, DataType: DataType.AppString, width: 150 },
          { Key: 'RDT', Name: '发起日期', IsShow: true, IsShowMobile: true, DataType: DataType.AppDateTime, width: 150 },
          { Key: 'WFState', Name: '标签', IsShow: true, IsShowMobile: true, DataType: DataType.AppInt, width: 150 },
        ];
    ...
    const data: any = await handler.DoMethodReturnJson('Confim_False_Init');
