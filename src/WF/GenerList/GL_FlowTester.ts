import { HelpShowModel, PageBaseGenerList } from '../../bp/UIEntity/PageBaseGenerList';
...
import { GetParamsUrl, GetUrlToJSON } from '/@/utils/gener/StringUtils';
...
export class GL_FlowTester extends PageBaseGenerList {
  ...
  async Init() {
    ...
    this.BtnOfToolbar = '设置流程发起人,更多发起人,测试参数';
    ...
    this.Columns = [
      ...
      { Key: 'FK_DeptText', Name: '所在部门', IsShow: true, DataType: 1, width: 66 },
    ...
    const data: any = await handler.DoMethodReturnJson('TestFlow2020_Init');
    data.forEach((en) => {
      en.Btns = 'PC发起,Mobile发起';
    });
    this.Data = data;
  }
  ...
  async LinkFieldClick(record: Record<string, any>) {
    return this.FlowStart(record, 'PC发起');
  }
  ...
  async BtnClick(btnName: string, record: Record<string, any>) {
    ...
    if (btnName === '测试参数') {
      const url = GloComm.UrlEnOnly('TS.WF.Template.FlowTestParaSetting', flowNo);
 ...
 帐号就是登录该系统的编号，如果输入的帐号没有发起该流程的权限，系统就会提示错误。`;
      ...
      //让数据重新绑定.
      return new GPNReturnObj(GPNReturnType.Reload, this.Data);
    }
    if (btnName === '更多发起人') {
      const en = new FlowExt(flowNo);
      await en.RetrieveFromDBSources();
    ...
    if (btnName === 'PC发起') {
      return await this.FlowStart(record, btnName);
    }
    if (btnName === 'Mobile发起') {
      return await this.FlowStart(record, btnName);
    }
  ...
  async FlowStart(record, btnName) {
    const flowNo = this.RequestVal('FlowNo');
    //存储Admin的Token
    ...
    try {
      ...
      resultData = await handler1.DoMethodReturnString('Default_Init');
      ...
      if(typeof resultData === 'object'){
          const pageName: string = resultData['PageName'] || '';
          delete resultData['DoMethod'];
          delete resultData['DoType'];
          delete resultData['HttpHandlerName'];
          delete resultData['PageName'];
          delete resultData['TesterNo'];
          delete resultData['Token'];
          delete resultData['FlowNo'];
          url ='/#/WF/TestingContainer/Default?TesterNo=' +result.TesterNo +'&FlowNo=' +flowNo 
            +'&CurrPage=StartGuide&Token=' +result.Token +'&PageName='+pageName+'&' + GetParamsUrl(resultData)+'&t=' +Math.random();
      ...
      if (btnName == 'PC发起') {
        url =
          ...
          '&WorkID=' +
          resultData +
          '&CurrPage=MyFlow&Token=' +
          result.Token +
          '&t=' +
          Math.random();
      } else if (btnName == 'Mobile发起') {
        url =
          '/#/WF/TestingContainer/Default?TesterNo=' +
          result.TesterNo +
          '&FlowNo=' +
