<template>
  <BaseComponent ref="baseComponent" :update-func="InitPage" :close-drawer-func="InitPage">
      ...
      <ThemeWrapper>
        <Spin :spinning="loading" style="background-color: white; margin: 0px; height: 100%">
          ...
          <div v-else>
            <Card style="border-radius: 10px; margin-bottom: 6px">
              <div class="flex">
                <Form layout="inline" :label-col="{ span: 8 }" :wrapper-col="{ span: 20 }" style="width: 100%">
                  <FormItem margin-right="0">
                    <Input v-model:value="searchKey" :placeholder="'请输入关键字'" @change="Search" allow-clear style="width: 200px">
                      <template #prefix>
                        <SearchOutlined />
                      </template>
                    </Input>
                  ...
                  <FormItem style="margin-right: 0" v-for="col in searchColumns" :key="col.key">
                    <Select v-model:value="col.value" style="width: 200px" @change="Search" :placeholder="'请选择' + col.title" :allowClear="true">
                      <SelectOption v-for="item in col.options" :key="item.key" :label="item.label" :value="item.value">{{ item.label }}</SelectOption>
                    </Select>
                  </FormItem>
                  <template v-if="showRDT">
                    <!-- <FormItem :label="dtFieldOfSearchLab"> -->
                    <RangePicker v-model:value="searchDt" :placeholder="'[`${dtFieldOfSearchLab}从`, `到`]'" @change="Search" style="width: 250px" />
                    <!-- </FormItem> -->
                  </template>
                  <FormItem>
                ...
                <div class="toolbar_toRight">
                  ...
                  <template v-if="showModel === 0 && defaultStyle.Type === GenerListPageShowModel.Table">
                    <Dropdown :trigger="['click']">
                      <Button type="primary" style="margin-right: 15px">...</Button>
                      <template #overlay>
                        <Menu>
                          <MenuItem
              ...
              <div class="btn-grid">
                <Button v-for="btn in toolBars" :key="btn" type="primary" :class="btn_style(btn)" @click="toolBarOper(btn)">{{ btn }}</Button>
              </div>
            </Card>
            <BasicTable
            ...
            <div v-else-if="showModel === GenerListPageShowModel.Windows || showModel === 0" style="height: calc(100vh - 50px); overflow: hidden auto" class="WinPanel">
              <template v-if="defaultGroupField != ''">
                  ...
                  <Row v-if="defaultStyle.Type == 2" :gutter="[8, 8]" style="margin-bottom: 10px">
                    <Col :span="8" v-for="item in tableData">
                      <Collapse expandIconPosition="end" v-model:activeKey="item[columns[0].key]">
                        <CollapsePanel :key="item[columns[0].key]" :header="item[columns[0].key]" collapsible="disabled" class="ant-panel">
                    ...
                    <Row :gutter="[24, 24]" style="margin-bottom: 10px">
                      <Col :span="24" v-for="item in tableData">
                        <div class="Start_cont">
                          <div class="Start_title"> {{ item[columns[0].key] }}</div>
                            ...
                            <div v-for="child in item.children" class="Start_layout_cont">
                              <div style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap">
                                <Button type="link" @click="LinkFieldClick(child)" style="color: #096dd9; display: flex; align-items: center">
                                  ...
                                  <!-- <i :class="child.Icon" class="Start_Icon" :style="{ fontSize: defaultStyle.Type == 2 ? '13px' : defaultStyle.Type == 3 ? '25px' : '13px' }"></i> -->
                                  {{ child[linkField] }}</Button
                                >
                              </div>
                              <Popover
...
<script lang="tsx" setup>
  import { h, nextTick, reactive, ref, shallowRef, UnwrapRef, VNodeChild, computed } from 'vue';
  ...
  import { ColumnHeightOutlined, ToolOutlined, PrinterOutlined, SearchOutlined } from '@ant-design/icons-vue';
  ...
  const handleSetStyle = async (value: any, key: string) => {
    ...
    if (!!GetEnName.value && GetEnName.value !== '') {
      setCookie(WebUser.No + '_' + GetEnName.value, defaultStyle.Type.toString(), 5);
      setCookie(WebUser.No + '_' + GetEnName.value + '_Type', key.key, 5);
    }
    await InitPage();
  };
  ...
  const InitPage = async () => {
    try {
      ...
      const getStyleType = parseInt(getCookie(WebUser.No + '_' + GetEnName.value));
      const getStyle = getCookie(WebUser.No + '_' + GetEnName.value + '_Type');
