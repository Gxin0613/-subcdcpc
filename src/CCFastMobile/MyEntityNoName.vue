...
<script setup lang="ts">
  ...
  const InitToolbar = async () => {
    ...
    //草稿状态.
    if (dictState == '0') {
      frmBtn.NewEnable = 1; //新建.
      frmBtn.SaveEnable = 1; //保存.
      frmBtn.DeleteEnable = 1; //删除.
      frmBtn.FilingDoneEnable = 0; //归档.
      frmBtn.FilingUnEnable = 0; //撤销归档.
    }
    ...
    if (dictState == '3') {
      frmBtn.NewEnable = 1; //新建.
      frmBtn.SaveEnable = 0; //保存.
      frmBtn.DeleteEnable = 0; //删除.
      frmBtn.FilingDoneEnable = 0; //归档.
      frmBtn.FilingUnEnable = 1; //撤销归档.
    }
  ...
  const GetFormData = async () => {
    ...
    handler.AddPara('RefNo', originalProps.No);
