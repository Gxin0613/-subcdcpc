    ...
    <VantCheckGroup v-model="tableConfigs.checkedItems" style="padding-top: 46px; padding-bottom: 40px">
      ...
      <VantCellGroup inset>
        <VantList
          ...
          @load="loadMore"
        >
          <div v-for="(item, index) in tableConfigs.dataSource" class="card-item" center :key="item.No" clickable>
            <div class="title">
              <VantCheckbox :name="item.No" :ref="(el) => (checkboxRefs[index] = el)" @click.stop="toggle(index)" />
...
<script lang="ts" setup>
  ...
  import { onBeforeUpdate, reactive, Ref, ref, shallowRef } from 'vue';
  ...
  import { useSearchNoName } from '../CCFast/CCBill/hooks/useSearchNoNameMobile';
  ...
  const { getParams, query, loadMore, resetUserRegedit, InitUserRegedit, InitMapAttrs, InitToolbar, updateUserRegedit, toolbarProps, tableConfigs } = useSearchNoName(
    'BP.CCBill.WF_CCBill',
    props,
    'Search_Init',
    popup,
