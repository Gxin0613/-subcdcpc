<template>
  <BaseComponent ref="baseComponent" :close-drawer-func="InitSearch" :close-modal-func="InitSearch" style="background-color: #f2f5f7">
    <VantCheckGroup v-model="tableConfigs.checkedItems" style="padding-top: 46px; padding-bottom: 40px">
      ...
      <VantEmpty v-else-if="tableConfigs.dataSource.length === 0" :description="'暂无数据'" style="height: calc(-150px + 100vh); padding: 10px 10px; background-color: white">
      </VantEmpty>
      <VantCellGroup inset>
        <VantList
          style="height: calc(100vh - 160px); overflow-y: auto; background-color: #f2f5f7"
...
<script lang="ts" setup>
  ...
  import { useSearchDict } from '../CCFast/CCBill/hooks/useSearchBillMobile';
  ...
  const InitSearch = async () => {
    try {
      ...
      await InitToolbar();
      if (displayMode.value === 'table') await InitMapAttrs();
      simpleColumns.value = [...tableConfigs.columns.slice(0, 8).map((col) => ({ key: col.key, title: col.title }))];
      await query();
    } catch (e: any) {
