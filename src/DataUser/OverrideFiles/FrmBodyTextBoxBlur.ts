 ...
 * 1. 事件类: 当文字变化的时候.
 */
export class FrmDtlTextBoxBlur {
  constructor() {}
  ...
  public static async Blur(pkval: number, dtlEnName: string, dtlOID: number, attrKey: string, rowData: Record<string, any>, tableData: any[]) {
    if (dtlEnName == 'ND101Dtl1' && (attrKey == 'SLXX' || attrKey == 'SLSX')) {
      //获得批次ID.
      const batchID = FrmDtlTextBoxBlur.GetTextBoxVal(rowData, 'BatchID');
      const textVal = FrmDtlTextBoxBlur.GetTextBoxVal(rowData, attrKey); //获得输入的值.
      tableData.forEach((item) => {
        if (item['BatchID'] === batchID) {
          //  @TextBoxChange="TextBoxChange".SetTextBoxVal(item, attrKey, textVal); //设置输入的值.
          FrmDtlTextBoxBlur.SetTextBoxVal(item, attrKey, textVal); //设置输入的值.
        }
      });
      return tableData;
    }
