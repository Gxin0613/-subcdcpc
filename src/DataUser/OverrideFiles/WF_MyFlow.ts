 ...
 */
// import { getQuarterString } from 'naive-ui/es/date-picker/src/utils';
// import { getQuarterString } from 'naive-ui/es/date-picker/src/utils';
export function userMyFlowSelfLoader(params) {
  /*
   ...
   * @constructor
   */
  const beforeSend = async (_row: any, _frmID: string, _flowNo: string, _nodeID: number, _workID: number) => {
    return true;
  };
   ...
   * 退回前事件
   */
  const beforeReturn = (_flowNo: string, _nodeID: number, _workID: number, _toNodeID: number, _toEmp: string) => {
    return true;
  };
   ...
   * @constructor
   */
  const WindCloseBySelf = () => {};
  /**
   * 删除前事件
   */
  const beforeDelete = (_row: any, _frmID: string, _flowNo: string, _nodeID: number, _workID: number) => {
    return true;
  };
  return {
    beforeSave,
