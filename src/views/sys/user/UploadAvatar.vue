<template>
  <div class="change-avatar">
    <CropperAvatar
      :uploadApi="uploadApi"
      :value="avatar"
      :btnText="$tt('views.sys.user.uploadavatar._key2')"
...
<script setup lang="ts">
  ...
  const avatar = computed(() => {
    ...
    const avatar = basicPath + 'DataUser/UserIcon/' + No + '.png?t=' + Math.random();
    return avatar;
  });
  ...
  const uploadApi = async (uploadParams: { file: Blob; name: string; filename: string; fileSrc?: File | undefined; btnText?: string }) => {
    ...
    if (btnText == translateText('views.sys.user.uploadavatar._key1')) {
