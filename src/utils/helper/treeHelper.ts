...
// 过滤Tree函数
export function filterTreeByNo(tree: any[], noToRemove: string): Recordable[] {
  return tree
    .filter((node) => node.No !== noToRemove)
    .map((node) => ({
      ...node,
      children: node.children ? filterTreeByNo(node.children, noToRemove) : undefined,
    }));
}
