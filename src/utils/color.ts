...
export const getConfigColor = (row, attr, config_list) => {
  const _cfg_list = config_list.filter((cfg) => cfg.AttrOfOper == attr.Key);
  if (_cfg_list.length > 0) {
    let _font_color = '';
    for (const _cfg of _cfg_list) {
      const _color = _cfg.Tag;
      const _min = parseFloat(_cfg.Tag1);
      const _max = parseFloat(_cfg.Tag2);
      const _val = parseFloat(row[attr.Key] as string);
