<template>
  <div>
    <Spin :spinning="loading" class="example">
          ...
          <template v-if="displayMode == 'normalMode'">
            <div>
              <div v-for="track in tracks" :key="track.MyPK">
                ...
                />
                <WorkCheckParse v-else ref="workCheckParse-R" :track="track" :frmWorkCheck="frmWorkCheck" :params="props.params" :is-readonly="props.isReadonly" />
              </div>
            </div>
          </template>
...
<script lang="ts" setup>
  ...
  console.log('frmData=', props.frmData);
  //错误提示
  ...
  const InitPage = async () => {
    try {
      ...
      if (typeof data.value === 'string' && data.value.includes('err@')) {
        showFailToast(data.value.replace('err@', ''));
        return;
      }
      ...
      if (displayMode.value == 'trackMode') {
        for (let i = 0; i < tracks.value.length; i++) {
          console.log('tracks.value[i]', tracks.value[i]);
      ...
      if (!!result.MyTrack) myTrack.value = !!result.MyTrack ? result.MyTrack[0] : null;
      ...
      console.log('trackWorks.value', trackWorks.value);
      //end

      // trackWorks.value = trackWorks.value.filter((trackItem) => trackItem.EmpFrom != '' || trackItem.EmpTo != '');
      ...
      //end
      // myTrack.value = !!result.MyTrack ? result.MyTrack[0] : null;
    } catch (e) {
  ...
  const WorkCheckSave = async (isSaveOnly) => {
    ...
    if (props.examineMode == 'trackTimeMode') {
      ...
      if (!!refT && typeof refT?.SaveWorkCheck === 'function') {
    ...
    if (!!refW && typeof refW?.SaveWorkCheck === 'function') {
