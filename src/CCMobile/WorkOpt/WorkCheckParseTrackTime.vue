<template>
  <div v-if="isShow" class="wc-wrapper">
    <div v-if="myTrack != null" style="border: 1px solid #d9d9d9; padding: 10px">
      <Row>
        <!-- <p v-if="props.isShowNodeName" style="margin-bottom: 8px; font-weight: 600; font-size: 16px; color: #6d6969">{{ trackInfo.NodeName }}</p> -->
        <Col :span="24">
          <Textarea
            v-model:value="workCheckDoc"
            placeholder="内容不能为空,请输入信息,或者使用常用短语选择,内容不超过2000字."
            :rows="3"
            style="width: 100%; border: none; height: 96px"
        ...
        <Col :span="24">
          <!-- 快速录入 -->
          <div style="margin-top: 12px; margin-right: 20px; cursor: pointer" @click="IsShow">
            <menu-fold-outlined v-if="isHideMsg" class="trigger" />
            <menu-unfold-outlined v-else class="trigger" />
            {{ isHideMsg ? '展开' : '收起' }}
          </div>
          <div v-if="!isHideMsg" style="margin-top: 12px; display: flex; align-items: center; flex-wrap: wrap">
            ...
            <div style="color: #3a64fe" @click="OpenUseExpresFlow">修改</div>
          </div>
    ...
    <div style="margin-top: 20px">
      <Timeline mode="left">
        <TimelineItem v-for="trackItem in trackLists" :key="trackItem.MyPK" style="width: 100%">
          <template v-if="props.isShowNodeName" #label>
            <p style="font-weight: 600; font-size: 12px; text-align: right">
              <template v-if="!!trackItem.NodeName">
                ...
                <span v-else-if="ActionType.CC === trackItem.ActionType" class="font">抄送</span>
          ...
          <Row style="display: flex" flexFlow="unset" v-for="nodeTrack in trackItem.NodeArr" :key="nodeTrack.MyPK">
            <Col :span="24" style="width: 100%; background-color: #f5f5f5; margin-bottom: 5px; padding: 5px 0; border-radius: 10px">
              <div style="margin-left: auto; padding: 0 10px; margin-bottom: 5px; display: flex; justify-content: space-between; font-size: 14px" class="setPadding">
                <template v-if="nodeTrack.EmpFromT">
                  <span class="CehckPerson">
                    <template v-if="nodeTrack.SigantureEnabel === 0">
                      ...
                      <div>{{ nodeTrack.EmpFromT }}</div>
                    </template>
                    <template v-else-if="nodeTrack.SigantureEnabel === 1"> <img :src="getSiganTureImg(nodeTrack.EmpFrom)" :onerror="defaultImg" />: </template>
                    <template v-else-if="nodeTrack.SigantureEnabel === 2"> <img :src="nodeTrack.WritImg" style="display: inline; height: 40px; margin-top: -15px" /></template>
              ...
              <template v-if="nodeTrack.ActionType === ActionType.Return || nodeTrack.ActionType === ActionType.ReturnAndBackWay">
                <div style="text-align: end; color: #d81e08" class="setPadding">退回给{{ nodeTrack.EmpToT }}</div>
              </template>
              <template v-if="nodeTrack.ActionType != ActionType.CC">
                <div v-if="nodeTrack.ActionType === ActionType.RebackOverFlow" class="setPadding">回滚信息:{{ nodeTrack.Msg }}</div>
                <div v-else-if="nodeTrack.ActionType === ActionType.FlowOver && nodeTrack.Msg != '流程结束'" class="setPadding"></div>
                <div v-else class="setPadding">{{ nodeTrack.Msg }}</div>
                <template v-if="!!trackItem.FWCMsg"
                  ><p class="fwcMsg">审核立场：{{ trackItem.FWCMsg }}</p></template
                >
              </template>
              <div v-if="!nodeTrack.NodeID.toString().endsWith('01') && nodeTrack.HS" style="text-align: right; font-size: 14px" class="setPadding">
                <template v-if="nodeTrack.ActionType === ActionType.CC">
                  <template v-for="trackCC in nodeTrack?.cclist" :key="trackCC.MyPK">
                    <div v-if="trackCC.Sta == 1" class="NodeTime"> {{ trackCC.CCToName }} - 耗时:{{ dwellTime(trackCC.ReadDT, trackCC.RDT) }},</div>
                  </template>
                </template>
                <template v-else
                  ><span class="NodeTime"> 耗时:{{ nodeTrack.HS }}</span>
                </template>
              </div>
              <template v-if="nodeTrack.ActionType === ActionType.Shift">
                <div style="color: blue" class="setPadding">移交给{{ nodeTrack.EmpToT }}</div>
              </template>
              <template v-if="nodeTrack.ActionType === ActionType.CC">
                <template v-for="trackCC in nodeTrack?.cclist" :key="trackCC.MyPK">
                  <div class="setPadding"> {{ trackCC.CCToName }}-{{ trackCC.StaText }},</div>
                </template>
              </template>

              <div style="display: flex; justify-content: space-between; border-top: 1px solid #cecece" class="setPadding">
                <span
                ...
                <span v-else-if="trackItem.ActionType === ActionType.Return || nodeTrack.ActionType === ActionType.ReturnAndBackWay" style="color: #d81e08" class="NodeNameStyle"
                  >退回</span
                >
                <span v-else-if="nodeTrack.ActionType === ActionType.WorkCheck" style="color: #8dd000" class="NodeNameStyle">审核意见</span>
                <span v-else-if="nodeTrack.Msg === '未审批' && nodeTrack.ActionType === ActionType.CC" style="color: #7f7f7f" class="NodeNameStyle"> 未开始</span>
                <span v-else-if="nodeTrack.ActionType === ActionType.CC" style="color: #ff9933" class="NodeNameStyle"> 传阅 </span>
                <span v-else-if="nodeTrack.ActionType === ActionType.TeampUp" style="color: #8dd000" class="NodeNameStyle">协作</span>
                <span v-else-if="nodeTrack.ActionType === ActionType.ForwardFL" style="color: #8dd000" class="NodeNameStyle">分流前进</span>
                <span v-else-if="nodeTrack.ActionType === ActionType.ForwardHL" style="color: #8dd000" class="NodeNameStyle">合流前进</span>
                <span v-else-if="nodeTrack.ActionType === ActionType.Shift" style="color: #8dd000" class="NodeNameStyle">移交</span>
                <span v-else-if="nodeTrack.ActionType === ActionType.HuiQian" style="color: #8dd000" class="NodeNameStyle">加签</span>
                <span v-else-if="nodeTrack.ActionType === ActionType.UnSend" style="color: #d81e08" class="NodeNameStyle"> 撤销发送</span>
                ...
                <span v-if="nodeTrack.NodeID" class="NodeTime">{{ nodeTrack.RDT }} </span>
              </div>
            </Col>
...
<script lang="ts" setup>
  ...
  const positionList = ref<Recordable[]>([]);
  //弹窗显示
  ...
  const InitPage = async () => {
    ...
    props.trackList.forEach((track) => {
      if (track.ActionType === ActionType.CC) {
        track.cclist = cc.filter((item) => item.NodeIDCC.toString() === track.NDTo.toString() && item.NodeIDWork.toString() === track.NDFrom.toString());
        if (track.Msg === '未审批') {
...
<style scoped>
  :deep(.ant-timeline .ant-timeline-item-label) {
    width: 10%;
  }
  :deep(.ant-timeline .ant-timeline-item-head) {
    left: 15%;
  }
  :deep(.ant-timeline .ant-timeline-item-left .ant-timeline-item-content) {
    left: 15%;
    width: calc(80% - 10px);
    text-align: left;
  }
