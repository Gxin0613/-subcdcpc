<template>
  <div class="list-picker">
    <div class="header">
      <Checkbox v-model="checkAll" @click="onCheckAllChange" :disabled="!isMultiSelect">{{'全选'}}</Checkbox>
    </div>
    <br />
    <CheckboxGroup v-model="checkedList">
      <CellGroup inset>
        <Cell v-for="item in plainOptions" clickable :key="item.No" :title="item.Name" @click="singleCheckboxChange(item)">
          <template #right-icon>
            <Checkbox :name="item.No" />
          </template>
        </Cell>
      </CellGroup>
    </CheckboxGroup>
...
<script lang="ts" setup>
  ...
  const onCheckAllChange = () => {
    checkedList.value = checkAll.value ? plainOptions.value.map((item: CheckedItem) => item.No) : [];
  };
  const singleCheckboxChange = (item) => {
