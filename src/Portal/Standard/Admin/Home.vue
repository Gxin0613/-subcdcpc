<template>
  <Layout>
    <LayoutSider v-model:collapsed="collapsed" :trigger="null" collapsible style="background: #fff">
      <div class="logo" v-if="collapsed">{{ '集团版' }}</div>
      <div class="logo" v-else>{{ VITE_GLOB_SX_TITLE }}BPM集团版</div>
      <Menu v-model:selectedKeys="selectedKeys" mode="inline" v-model:openKeys="openKeys">
        <SubMenu v-for="group in GroupMenuList" :key="group.GroupNo">
          <template #icon>
      ...
      >
        ...
        <div class="quick-links">
          <Menu mode="horizontal" v-model:selectedKeys="selectedKeys" style="background: var(--system-bg-color); height: 48px; font-size: 14px">
            <menu-item key="TreeEns_Dept2Emp" @click="addTabByMenu({ path: '/WF/GL/TodoList' })" class="cg_style">
              <i class="icon-arrow-left-circle"></i>{{ '返回' }}</menu-item
            >
            <menu-item key="200-02" class="quitOut cg_style" @click="LoginOut"> <LogOutOutline style="width: 16px" />{{ '退出' }}</menu-item>
          </Menu>
        </div>
      </LayoutHeader>
      <LayoutContent :style="{ margin: '0px', padding: '8px 6px', background: '#f2f5f7', minHeight: '280px' }">
...
<script lang="ts" setup>
  ...
  import GroupMenuList from './GroupMenu';
