      ...
      <div class="know-all-cont">
        ...
        <div class="know-side">
          <div class="know-side-nav">
            <!-- 标题/返回键 -->
            <H3><i class="icon-home move-enter"></i> {{ cateinfoData?.Title }}</H3>
          </div>
          <div class="know-side-menu">
            <div class="menu-system sideHover">
              <H4 class="little-title-cont">
                <i class="icon-layers move-enter font-cont"> 知识库结构 </i>
              </H4>
              <Dropdown :trigger="['click']">
                <i class="icon-plus move-enter font-cont" @click="cateShow = true"></i>
                <template #overlay>
                  <Menu>
        ...
        <div class="know-page-main">
          ...
          <div v-if="flieType == 1">
            <div class="nav">
              ...
              <div class="nav-right move-enter" style="color: red" @click.stop="handleDeleteClick(treeSelectedInfo)"> <DeleteOutlined />删除 </div>
            </div>
                ...
                <TabPane key="1" tab="全部文档">
                  <List bordered :data-source="treeSelectedInfo?.children">
                    <template #renderItem="{ item }">
                      <ListItem>
                        <div class="list-item move-enter" @click="selectFileInfo(item)">
          ...
          <div v-if="flieType == 2">
            <div class="nav">
              <div class="nav-left move-enter">
                <div class="cate-info"> <FileOutlined />{{ treeSelectedInfo.Name }} 创建于{{ treeSelectedInfo.RDT }} </div>
              </div>
              <div class="nav-right">
                <span class="upload-btn move-enter" @click="uploadAth(treeSelectedInfo)"><UploadOutlined />上传附件</span>
                <span class="cate-btn move-enter" @click="openTinymce(treeSelectedInfo)"><EditOutlined />编辑</span>
                <span class="delete-btn move-enter" @click="handleDeleteClick(treeSelectedInfo)"><DeleteOutlined />删除</span>
              </div>
            </div>
            <div class="cate-info-cont">
              ...
              <template v-for="db in athList" :key="db.MyPK">
                <div class="db-list">
                  ...
                  <div class="db-file-info">
                    ...
                    <span class="db-info-fun move-enter">
...
<script setup lang="ts">
  ...
  import { downloadImageWithWatermark } from '/@/utils/watermark/index';
  ...
  const viewParams = ref(props.viewParams);
  const KnNo = ref(viewParams.value?.No);
  ...
  const nodeProps = ({ option }: { option: TreeOption }) => {
    return {
      onClick: async () => {
        ...
        console.log('treeSelectedKeys.value', treeSelectedKeys.value);
  ...
  const selectFileInfo = async (item) => {
//    debugger;
    treeSelectedKeys.value = item.key;
  ...
  const customRequest = async ({ file, onSuccess }) => {
    ...
    handler.AddPara('RefPKVal', treeSelectedKeys.value);
  ...
  const AthList = async () => {
    ...
    await ens.Retrieve('RefPKVal', treeSelectedKeys.value, 'Sort', 'Knowledge_MinAth');
    //判断是否可以删除附件
    ...
    // }
  //  debugger;
    for (const ath of ens?.getData()) {
      console.log('ath.RefPKVal', ath.RefPKVal);
      console.log(treeSelectedKeys.value);
      if (ath.RefPKVal == treeSelectedKeys.value) {
        athList.value.push(ath);
      }
    }
  };
  ...
  const dbDownload = (db) => {
   ...
   // debugger;
    const waterText = viewParams.value?.WaterText === '' ? '涉密' : viewParams.value?.WaterText;
    if (isImageFile(fileName) && viewParams.value.IsShowWaterMark === '1') {
      downloadImageWithWatermark(url, waterText, fileName);
    } else {
