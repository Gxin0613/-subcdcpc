...
<script lang="ts" setup>
  ...
  const EnName = props.params.FrmID || props.params.FrmID || (route.query.FrmID as string);
  ...
  const InitColumn = (mapAttrs) => {
    ...
    columns.value = [];
    columns.value = mapAttrs
      .filter((mapAttr: MapAttr) => mapAttr.UIVisible && !hideAttrs.includes(mapAttr.KeyOfEn))
      .map((mapAttr: MapAttr) => {
        const obj: Recordable = {
          title: mapAttr.Name,
          width: mapAttr.UIWidth < 50 ? 50 : mapAttr.UIWidth,
          key: mapAttr.KeyOfEn,
        };
        if (mapAttr.MyDataType === DataType.AppDate || mapAttr.MyDataType === DataType.AppDateTime) {
          obj.width = 150;
          obj.render = (row) => {
            let time = row[mapAttr.KeyOfEn];
        ...
        if (mapAttr.UIContralType == UIContralType.RadioBtn || mapAttr.UIContralType == UIContralType.DDL) {
          obj.render = (row) => {
            const textBox = row[mapAttr.KeyOfEn + 'Text'] || row[mapAttr.KeyOfEn + 'T'];
            return h('span', textBox);
          };
        }
        if (mapAttr.MyDataType === DataType.AppBoolean) {
          obj.render = (row) => {
            const textBox = row[mapAttr.KeyOfEn] == 1 ? '是' : '否';
            return h(
              'span',
