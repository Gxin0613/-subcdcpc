<template>
  <Tooltip placement="top">
    ...
    <Popover placement="bottomLeft" trigger="click" @open-change="onOpenChange" :overlayClassName="`${prefixCls}__column-list`" :getPopupContainer="getPopupContainer">
      <template #title>
...
<script lang="ts" setup>
  ...
  import { cloneDeep, omit, isNil } from 'lodash-es';
  ...
  const diff = () => {
    if (typeof route.name === 'string') {
      ...
      if (cache) {
        // value、label是否一致
        if (JSON.stringify(columnOptions.value.map((o) => ({ value: o.value, label: o.label }))) !== JSON.stringify(cache.map((o) => ({ value: o.value, label: o.label })))) {
