...
<script lang="ts">
  ...
  type GetCroppedCanvasOptions = Cropper.GetCroppedCanvasOptions;

  const CroppedCanvasOptions: GetCroppedCanvasOptions = {
    fillColor: '#fff',
  };
  ...
  export default defineComponent({
    ...
    setup(props, { attrs, emit }) {
      const defaultOptions: Options = {
        aspectRatio: props?.aspectRatio as number, //设置裁剪框为固定的宽高比
        zoomable: true,
        zoomOnTouch: true,
        zoomOnWheel: true,
        cropBoxMovable: true,
      ...
      function croppered() {
        ...
        const canvas = props.circled ? getRoundedCanvas() : cropper.value.getCroppedCanvas(CroppedCanvasOptions);
        canvas.toBlob((blob) => {
          if (!blob) {
            return;
          }
