  ...
  export default defineComponent({
    ...
    setup(props, { emit, attrs }) {
      ...
      const handleImageUpload = (blobInfo, success, failure, progress) => {
        ...
        Dev2Interface.Upload_File(blobInfo.blob(), Date.now() + '_' + blobInfo.name() + '.png', 'rich_editor')
          .then((url) => {
            success(VITE_GLOB_API_URL + url);
          })
          .catch((err) => {
            failure(err);
          });
      ...
      const initOptions = computed((): RawEditorSettings => {
        ...
        return {
          ...
          content_css: publicPath + 'resource/tinymce/skins/ui/' + skinName.value + '/content.min.less',
          ...options,
          setup: (editor: Editor) => {
      ...
      function bindModelHandlers(editor: any) {
        ...
        editor.on(normalizedEvents ? normalizedEvents : 'change keyup undo redo', () => {
          ...
          // const validContent = content.replace(/\"/g, "'");
          // emit('update:modelValue', validContent);
          // emit('change', validContent);
      ...
      function handleDone(name: string, url: string) {
        ...
        const content = editor?.getContent() ?? '';
        const val = content?.replace(getUploadingImgName(name), `<img src="${url}" width="100%" />`) ?? '';
        setValue(editor, val);
      }
